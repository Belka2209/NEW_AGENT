...
Агент сам открывает **отдельный Chrome** с профилем `data/chrome-profile` (не вкладку чата http://127.0.0.1:8000). Обычный браузер закрывать не нужно. Скрипт `.\chrome-debug.ps1` больше не обязателен.
