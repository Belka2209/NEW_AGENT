...
Общее для обоих интерфейсов: модель из `.env`, долгая память, напоминания, логи, база `data/agent.db`, папка `workspace/`. Список чатов в боковой панели браузера и сессии `python -m app.cli` — разные диалоги, но память и напоминания одни.
