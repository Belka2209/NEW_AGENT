...
- Два режима: **чат** (погода, браузер, Telegram) и **код** (ревью, `edit_file`, git, команды). В браузере кнопки Авто / Чат / Код, в терминале `/chat` `/code` `/auto`
- Ревью: файл дочитывается порциями, затем один ответ; уточнения («что улучшить?») идут по тому же файлу
- На задачах про код — модель `OLLAMA_CODE_MODEL`
...
- Логи в `logs/agent.log` (на время разработки в git, чтобы смотреть сбои)
...
|---|---|
| `/new` | новый диалог |
| `/chat` `/code` `/auto` | режим: быт, код или по тексту |
| `/help` | список команд |
| `/exit` или `/q` | выход |
| Ctrl+C | выход |
...
Работает одинаково в браузере и в терминале. База в `data/agent.db` (не в git). Лог — `logs/agent.log`, на время разработки его коммитим.
...
**Логи** — кто какой инструмент вызвал и какие ошибки были. Пишется в `logs/agent.log`.

```powershell
Get-Content D:\MY_AGENT\NEW_AGENT\logs\agent.log -Tail 80
```

После сессии на RDP, чтобы лог был в репозитории:

```powershell
cd D:\MY_AGENT\NEW_AGENT
git add logs/agent.log
git commit -m "dev log"
git push
```

Ротация примерно 2 МБ, старые копии `logs/agent.log.1` в git не входят.
